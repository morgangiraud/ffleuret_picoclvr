    ...
    def tensorize(self, sequences):
        ...
        return torch.cat(
            [
                torch.tensor(
                    [
                        [self.token2id[str(c)] for c in s + ["<nul>"] * (len_max - len(s))]
                        for s in sequences
                    ]
                )
            ],
...
class Expr(Task):
    def tensorize(self, sequences):
        ...
        return torch.cat(
            [
                torch.tensor(
                    [[self.char2id[c] for c in s + "#" * (len_max - len(s))] for s in sequences]
                )
            ],
            0,
        ).to(self.device)
